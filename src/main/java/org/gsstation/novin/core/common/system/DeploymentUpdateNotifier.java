...
public class DeploymentUpdateNotifier {
        ...
        watcherThread = new Thread(() -> {
            while (keepWatching) {
                ...
                try {
                    ...
                    if (activitiesBuilder.length() > 0) {
                        if (NameRegistrar.getIfExists("main-logger") != null)
                            MainLogger.log(activitiesBuilder.toString(),
                                    THIS_CLASS_NAME);
                        /*else
    ...
    @SuppressWarnings("unused")
    public void watchDeployDir(ConfigurationUpdateListener<Path> listener)
            throws IOException {
        watch(GsMain.getQ2Instance().getDeployDir().toPath(), listener);
    }
    ...
    @SuppressWarnings("unused")
    public void stopOnDeployDir(ConfigurationUpdateListener<Path> listener)
            throws IOException {
        stopWatch(GsMain.getQ2Instance().getDeployDir().toPath(), listener);
    }
